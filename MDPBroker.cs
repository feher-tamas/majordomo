...
namespace Majordomo
{
    ...
    public class MDPBroker : IMDPBroker
    {
        ...
        private readonly List<Service> _services;              // list of known services
        private readonly List<Worker> _knownWorkers;           // list of all known workers, regardless of service offered

        private string _endpoint;                              // the endpoint the broker binds to
        private int _heartbeatLiveliness;                      // indicates the 'liveliness' of a worker
        private TimeSpan _heartbeatExpiry;                     // when a worker expires -> set by HeartbeatLiveliness(!)
        private readonly TimeSpan _heartbeatInterval;          // the time interval between heartbeats
        private bool _isBound;                                 // true if socket is bound to address
        private bool _isRunning;                               // true if the broker is running

        private readonly object _syncRoot = new();     // used as synchronization object for Purge ()
        ...
        {
            get { return _heartbeatLiveliness; }
            set
            {
                _heartbeatLiveliness = value;
                _heartbeatExpiry = TimeSpan.FromMilliseconds(HeartbeatInterval.TotalMilliseconds * value);
            }
        }
        ...
        private MDPBroker()
        {
            ...
            _heartbeatInterval = TimeSpan.FromMilliseconds(2500); // otherwise the expiry would be 0(!)
            ...
            : this()
        {
            ...
            if (heartbeatInterval > 0)
                _heartbeatInterval = TimeSpan.FromMilliseconds(heartbeatInterval);
        ...
        public void Bind()
        {
            ...
            {
                var error = $"The bind operation failed. Most likely because 'endpoint' ({_endpoint}) is malformed!";
                error += $"\nMessage: {e.Message}";
            ...
            Log($"MDP Broker/{major}.{minor} is active at {_endpoint}");
        }
        ...
        public void Bind([NotNull] string endpoint)
        {
            ...
            if (_endpoint == endpoint && _isBound)
                return;

            if (_isBound)
                Socket.Unbind(_endpoint);
        ...
        public void ProcessWorkerMessage([NotNull] NetMQFrame sender, [NotNull] NetMQMessage message)
        {
            ...
            var workerIsKnown = _knownWorkers.Any(w => w.Id == workerId);
            ...
            switch (cmd)
            {
                case MDPCommand.Ready:
                    if (workerIsKnown)
                    {
                        ...
                        RemoveWorker(_knownWorkers.Find(w => w.Id == workerId));
                ...
                case MDPCommand.Reply:
                    if (workerIsKnown)
                    {
                        var worker = _knownWorkers.Find(w => w.Id == workerId);
                ...
                case MDPCommand.Heartbeat:
                    ...
                    {
                        var worker = _knownWorkers.Find(w => w.Id == workerId);
                        worker.Expiry = DateTime.UtcNow + _heartbeatExpiry;
        ...
        public void ProcessClientMessage([NotNull] NetMQFrame sender, [NotNull] NetMQMessage message)
        {
            ...
            if (serviceName == "mmi.service")
            {
                ...
                if (_services.Exists(s => s.Name == name))
                {
                    var svc = _services.Find(s => s.Name == name);
        ...
        {
            var svc = _services.Exists(s => s.Name == serviceName)
                          ? _services.Find(s => s.Name == serviceName)
